using EjemploClases;
{
   Coche Ford = new Coche();
    Coche Mazda = new Coche();
    Coche Renault = new Coche();
    Coche Subaru = new Coche();
    Coche Mercedes = new Coche();
    Coche BMW = new Coche();
    Coche SEAT = new Coche();
    Coche Ferrari = new Coche();
    Coche Chevrolet = new Coche();
    Coche Citroen = new Coche();

    Ford.Cilindrada = "1200";


    Empleado empleado1 = new Empleado();
    empleado1.Nombre = "Pedro";
    empleado1.Departamento = "IT";
    empleado1.Telefono = "1234567";
    empleado1.IBAN = "23455";
    empleado1.NumeroEmpleado = "1";
    empleado1.NSS = "9876";

    Empleado empleado2 = new Empleado();
    empleado2.Nombre = "Maria";
    empleado2.Departamento = "Desarrollo de apps";
    empleado2.Telefono = "76543";
    empleado2.IBAN = "235655";
    empleado2.NumeroEmpleado = "2";
    empleado2.NSS = "987667";

    Empleado empleado3 = new Empleado();
    empleado3.Nombre = "Juan";
    empleado3.Departamento = "Recursos Humanos";
    empleado3.Telefono = "787543";
    empleado3.IBAN = "231655";
    empleado3.NumeroEmpleado = "3";
    empleado3.NSS = "987665";

    Empleado empleado4 = new Empleado();
    empleado4.Nombre = "Claudia";
    empleado4.Departamento = "Ventas";
    empleado4.Telefono = "23543";
    empleado4.IBAN = "235565";
    empleado4.NumeroEmpleado = "4";
    empleado4.NSS = "987437";

    Empleado empleado5 = new Empleado();
    empleado5.Nombre = "Olga";
    empleado5.Departamento = "Marketing";
    empleado5.Telefono = "72223";
    empleado5.IBAN = "2355685";
    empleado5.NumeroEmpleado = "5";
    empleado5.NSS = "987447";
    
    Empleado empleado6 = new Empleado();
    empleado6.Nombre = "Antoneta";
    empleado6.Departamento = "Directora";
    empleado6.Telefono = "123456";
    empleado6.IBAN = "2355666";
    empleado6.NumeroEmpleado = "6";
    empleado6.NSS = "987777";
}

